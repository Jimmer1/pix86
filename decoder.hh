...
StructuredUnaryOperands<I> structure_unary_operands(std::uint32_t (&R)[8], Memory& mem, const Operands& op) {
    ...
    if (op.rm.is_ptr) {
        ...
        if constexpr (std::is_same_v<I, std::uint8_t>) {
            if (op.rm.reg_high_8bit) {
                so.rm.r = GenericRegisterReference(&R[op.reg], get_low_word_high_byte, grh_set_low_word_high_byte);
            } else {
                so.rm.r = GenericRegisterReference(&R[op.reg], get_low_byte, grh_set_low_byte);
            }
        } else if constexpr (std::is_same_v<I, std::uint16_t> ) {
            so.rm.r = GenericRegisterReference(&R[op.reg], get_low_word, grh_set_low_word);
        } else {
            so.rm.r = GenericRegisterReference(&R[op.reg], null_getter, grh_null_setter);
        }
    }
    return so;
}
